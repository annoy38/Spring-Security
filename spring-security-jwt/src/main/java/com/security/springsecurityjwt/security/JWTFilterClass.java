...
public class JWTFilterClass extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            ...
            userEmail = utilClass.extractUsername(token);
        }

        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {

            UserDetails userDetails = customUserDetailServiceImp.loadUserByUsername(userEmail);

            if (utilClass.validateToken(token, userDetails)) {
